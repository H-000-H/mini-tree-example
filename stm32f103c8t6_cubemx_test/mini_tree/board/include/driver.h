 ...
 */
#ifdef __cplusplus
#define DRV_C_LINKAGE extern "C"
#else
#define DRV_C_LINKAGE
#endif

#define DRIVER_REGISTER(name, compat, probe_fn, remove_fn)        \
    DRV_C_LINKAGE int board_driver_probe_##name(device_t* dev)    \
    {                                                             \
        return probe_fn(dev);                                     \
    }                                                             \
    DRV_C_LINKAGE int board_driver_remove_##name(device_t* dev)   \
    {                                                             \
