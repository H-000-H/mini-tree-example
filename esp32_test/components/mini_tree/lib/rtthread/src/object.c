...
/* ESP32-S3 UART0 direct debug output */
#define OBJ_UART_FIFO   (*(volatile unsigned int*)0x60000000)
#define OBJ_UART_STATUS (*(volatile unsigned int*)0x6000001C)
#define OBJ_TXFIFO_CNT  (((OBJ_UART_STATUS >> 16) & 0x3FF))

static void obj_dbg_putchar(char c)
{
    while (OBJ_TXFIFO_CNT > 120) { }
    OBJ_UART_FIFO = c;
}
static void obj_dbg_puts(const char *s)
{
    while (*s) {
        if (*s == '\n') obj_dbg_putchar('\r');
        obj_dbg_putchar(*s++);
                    ...
                    const char               *name)
{
    ...
    obj_dbg_puts("[OBJ] rt_object_init entered\n");

    /* get object information */
    information = rt_object_get_information(type);
    obj_dbg_puts("[OBJ] rt_object_get_information returned\n");
    ...
    obj_dbg_puts("[OBJ] rt_object_init: setting params\n");
    ...
    obj_dbg_puts("[OBJ] locking spinlock\n");
    level = rt_spin_lock_irqsave(&(information->spinlock));
    obj_dbg_puts("[OBJ] locked, insert into list\n");
    ...
    obj_dbg_puts("[OBJ] list insertion done\n");
    rt_spin_unlock_irqrestore(&(information->spinlock), level);
    obj_dbg_puts("[OBJ] rt_object_init done\n");
}
